...
function dirfind_content(App $a, $prefix = "") {
	...
	$search = $prefix.Text::noTags(trim(defaults($_REQUEST, 'search', '')));
	...
	if (strpos($search,'@') === 0) {
		...
		if ((Text::validEmail($search) && Network::isEmailDomainValid($search)) ||
			(substr(Text::normaliseLink($search), 0, 7) == "http://")) {

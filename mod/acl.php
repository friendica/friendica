...
function acl_content(App $a)
{
	...
	if ($type == '') {
		...
		foreach ($r as $g) {
			$contacts[] = [
				'photo'   => Proxy::proxifyUrl($g['photo'], false, Proxy::SIZE_MICRO),
				'name'    => $g['name'],
	...
	if (DBA::isResult($r)) {
		...
		foreach ($r as $g) {
			$entry = [
				'type'    => 'c',
				'photo'   => Proxy::proxifyUrl($g['micro'], false, Proxy::SIZE_MICRO),
				'name'    => htmlentities($g['name']),
				'id'      => intval($g['id']),
				'network' => $g['network'],
				'link'    => $g['url'],
	...
	if ($conv_id) {
		...
		foreach ($item_authors as $author) {
			...
			if (count($contact) > 0) {
				$unknown_contacts[] = [
					'type'    => 'c',
					'photo'   => Proxy::proxifyUrl($contact['micro'], false, Proxy::SIZE_MICRO),

...
function dfrn_confirm_post(App $a, $handsfree = null)
{
	...
	if (!x($_POST, 'source_url')) {
		...
		if ($network === NETWORK_DFRN) {
			...
			if (($relation == Contact::CONTACT_IS_SHARING) || ($duplex)) {
			...
			if ($network === NETWORK_DIASPORA) {
				...
				if ($new_relation != Contact::CONTACT_IS_FOLLOWER) {
					$writable = 1;
				}
			}
		...
		if ((isset($new_relation) && $new_relation == Contact::CONTACT_IS_FRIEND)) {
	...
	if (x($_POST, 'source_url')) {
		...
		if (($relation == Contact::CONTACT_IS_FOLLOWER) || ($duplex)) {
			$new_relation = Contact::CONTACT_IS_FRIEND;
		}
		...
		if (DBA::isResult($r)) {
			...
			if ($combined['notify-flags'] & NOTIFY_CONFIRM) {
				$mutual = ($new_relation == Contact::CONTACT_IS_FRIEND);
				notification([
					'type'         => NOTIFY_CONFIRM,

...
function poke_init(App $a)
{
	...
	App::logger('poke: verb ' . $verb . ' contact ' . $contact_id, LOGGER_DEBUG);
	...
	if (!DBA::isResult($r)) {
		App::logger('poke: no contact ' . $contact_id);
		return;
	}
	...
	$arr['object'] .= '<link>' . Text::xmlify('<link rel="alternate" type="text/html" href="' . $target['url'] . '" />' . "\n");

	$arr['object'] .= Text::xmlify('<link rel="photo" type="image/jpeg" href="' . $target['photo'] . '" />' . "\n");

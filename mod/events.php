...
function events_post(App $a)
{
	...
	if (strcmp($finish, $start) < 0 && !$nofinish) {
		...
		AppR::baseUrl()->redirect($onerror_path);
	}
...
function events_content(App $a)
{
	...
	AppR::page()['htmlhead'] .= Renderer::replaceMacros($htpl, [
		'$module_url' => '/events',
		'$modparams' => 1,
		'$i18n' => $i18n,
	]);
	...
	if ($mode == 'view') {
		...
		if (DBA::isResult($r)) {
			...
			foreach ($r as $rr) {
				...
				if (empty($links[$j])) {
					$links[$j] = AppR::baseUrl() . '/' . AppR::args()->getCommand() . '#link-' . $j;
				}
			}
		}
		...
		AppR::page()->registerFooterScript(Theme::getPathForFile('asset/typeahead.js/dist/typeahead.bundle.js'));
		AppR::page()->registerFooterScript(Theme::getPathForFile('js/friendica-tagsinput/friendica-tagsinput.js'));
		AppR::page()->registerStylesheet(Theme::getPathForFile('js/friendica-tagsinput/friendica-tagsinput.css'));
		AppR::page()->registerStylesheet(Theme::getPathForFile('js/friendica-tagsinput/friendica-tagsinput-typeahead.css'));

		$o = Renderer::replaceMacros($tpl, [
			'$tabs'      => $tabs,
			'$title'     => L10n::t('Events'),
			'$view'      => L10n::t('View'),
			'$new_event' => [AppR::baseUrl() . '/events/new', L10n::t('Create New Event'), '', ''],
			'$previous'  => [AppR::baseUrl() . '/events/$prevyear/$prevmonth', L10n::t('Previous'), '', ''],
			'$next'      => [AppR::baseUrl() . '/events/$nextyear/$nextmonth', L10n::t('Next'), '', ''],
	...
	if (in_array($mode, ['edit', 'new', 'copy'])) {
		...
		if (!$cid && in_array($mode, ['new', 'copy'])) {
			$acl = ACL::getFullSelectorHTML(AppR::page(), $a->user, false, ACL::getDefaultUserPermissions($orig_event));
		} else {
			$acl = '';
		}

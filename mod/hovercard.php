...
function hovercard_content()
{
	...
	$nurl = Text::normaliseLink(GContact::cleanContactUrl($profileurl));
	if (!$nurl) {
		return;
	}
	...
	if (!count($contact) && local_user()) {
		$nurl = Text::normaliseLink($profileurl);
		$contact = Contact::getDetailsByURL($nurl, local_user());
	}
	...
	$profile = [
		...
		'about'    => $contact['about'],
		'network'  => Text::formatNetworkName($contact['network'], $contact['url']),
...
function get_template_content($template, $root = '')
{
	...
	$t = Text::getMarkupTemplate($template, $root);
	$filename = $t->filename;

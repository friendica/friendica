...
function dfrn_request_post(App $a)
{
	if (($a->argc != 2) || (!count($a->profile))) {
		Text::logger('Wrong count of argc or profiles: argc=' . $a->argc . ',profile()=' . count($a->profile));
		return;
	}
	...
	if ((x($_POST, 'localconfirm')) && ($_POST['localconfirm'] == 1)) {
		...
		if (local_user() && ($a->user['nickname'] == $a->argv[1]) && (x($_POST, 'dfrn_url'))) {
			$dfrn_url = Text::noTags(trim($_POST['dfrn_url']));
			...
			if (x($dfrn_url)) {
				...
				$r = q("SELECT * FROM `contact` WHERE `uid` = %d AND `nurl` = '%s' AND NOT `self` LIMIT 1",
					intval(local_user()),
					DBA::escape(Text::normaliseLink($dfrn_url))
				);
	...
	if (x($_POST, 'dfrn_url')) {
		...
		$real_name = x($_POST, 'realname') ? Text::noTags(trim($_POST['realname'])) : '';
		...
		Text::logger('dfrn_request: url: ' . $url . ',network=' . $network, LOGGER_DEBUG);
		...
		if ($network === Protocol::DFRN) {
			...
			if (is_array($contact_record)) {
				$ret = q("INSERT INTO `intro` ( `uid`, `contact-id`, `blocked`, `knowyou`, `note`, `hash`, `datetime`)
					...
					DBA::escape(Text::noTags(trim(defaults($_POST, 'dfrn-request-message', '')))),
					DBA::escape($hash),
					DBA::escape(DateTimeFormat::utcNow())
				);
			}
...
function dfrn_request_content(App $a)
{
	...
	if (x($_GET, 'dfrn_url')) {
		...
		$dfrn_url = Text::noTags(trim(hex2bin($_GET['dfrn_url'])));
		$aes_allow = x($_GET, 'aes_allow') && $_GET['aes_allow'] == 1 ? 1 : 0;
		$confirm_key = x($_GET, 'confirm_key') ? $_GET['confirm_key'] : "";

		// Checking fastlane for validity
		if (x($_SESSION, "fastlane") && (Text::normaliseLink($_SESSION["fastlane"]) == Text::normaliseLink($dfrn_url))) {
		...
		$tpl = Text::getMarkupTemplate("dfrn_req_confirm.tpl");
		...
		if ($a->profile['page-flags'] == Contact::PAGE_NORMAL) {
			$tpl = Text::getMarkupTemplate('dfrn_request.tpl');
		} else {
			$tpl = Text::getMarkupTemplate('auto_request.tpl');
		}

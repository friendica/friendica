...
function photos_init(App $a) {
	...
	if ($a->argc > 1) {
		...
		if ($ret['success']) {
			$photo_albums_widget = App::replaceMacros(Text::getMarkupTemplate('photo_albums.tpl'), [
		...
		$tpl = Text::getMarkupTemplate("photos_head.tpl");
...
{
	App::logger('mod-photos: photos_post: begin' , LOGGER_DEBUG);
	App::logger('mod_photos: REQUEST ' . print_r($_REQUEST, true), LOGGER_DATA);
	App::logger('mod_photos: FILES '   . print_r($_FILES, true), LOGGER_DATA);
	...
	if (!$owner_record) {
		...
		App::logger('photos_post: unable to locate contact record for page owner. uid=' . $page_owner_uid);
		killme();
	}
	...
	if ($a->argc > 3 && $a->argv[2] === 'album') {
		...
		if ($_POST['dropalbum'] == L10n::t('Delete Album')) {
			...
			if (!empty($_REQUEST['confirm'])) {
				...
				$a->page['content'] = App::replaceMacros(Text::getMarkupTemplate('confirm.tpl'), [
					'$method' => 'post',
		...
		if (strlen($rawtags)) {
			...
			if (count($taginfo)) {
				foreach ($taginfo as $tagged) {
					...
					$arr['object'] .= '<link>' . Text::xmlify('<link rel="alternate" type="text/html" href="' . $tagged[1] . '" />' . "\n");
					if ($tagged[3]) {
						$arr['object'] .= Text::xmlify('<link rel="photo" type="'.$p[0]['type'].'" href="' . $tagged[3]['photo'] . '" />' . "\n");
					}
					...
					$arr['target'] .= '<link>' . Text::xmlify('<link rel="alternate" type="text/html" href="' . System::baseUrl() . '/photos/' . $owner_record['nickname'] . '/image/' . $p[0]['resource-id'] . '" />' . "\n" . '<link rel="preview" type="'.$p[0]['type'].'" href="' . System::baseUrl() . "/photo/" . $p[0]['resource-id'] . '-' . $best . '.' . $ext . '" />') . '</link></target>';
	...
	App::logger('mod/photos.php: photos_post(): album= ' . $album . ' newalbum= ' . $newalbum , LOGGER_DEBUG);
	...
	$str_group_allow   = Text::perms2Str(is_array($group_allow)   ? $group_allow   : explode(',', $group_allow));
	$str_contact_allow = Text::perms2Str(is_array($contact_allow) ? $contact_allow : explode(',', $contact_allow));
	$str_group_deny    = Text::perms2Str(is_array($group_deny)    ? $group_deny    : explode(',', $group_deny));
	$str_contact_deny  = Text::perms2Str(is_array($contact_deny)  ? $contact_deny  : explode(',', $contact_deny));
	...
	if ($error !== UPLOAD_ERR_OK) {
		switch ($error) {
			...
			case UPLOAD_ERR_FORM_SIZE:
				notice(L10n::t('Image exceeds size limit of %s', Text::formatBytes(defaults($_REQUEST, 'MAX_FILE_SIZE', 0))) . EOL);
				break;
	...
	App::logger('photos: upload: received file: ' . $filename . ' as ' . $src . ' ('. $type . ') ' . $filesize . ' bytes', LOGGER_DEBUG);
	...
	App::logger('mod/photos.php: photos_post(): loading the contents of ' . $src , LOGGER_DEBUG);
	...
	if (!$image->isValid()) {
		App::logger('mod/photos.php: photos_post(): unable to process image' , LOGGER_DEBUG);
	...
	if (!$r) {
		App::logger('mod/photos.php: photos_post(): image store failed', LOGGER_DEBUG);
...
function photos_content(App $a)
{
	...
	if ($datatype === 'upload') {
		...
		$default_upload_box = App::replaceMacros(Text::getMarkupTemplate('photos_default_uploader_box.tpl'), []);
		$default_upload_submit = App::replaceMacros(Text::getMarkupTemplate('photos_default_uploader_submit.tpl'), [
			'$submit' => L10n::t('Submit'),
		]);

		$usage_message = '';

		$tpl = Text::getMarkupTemplate('photos_upload.tpl');
	...
	if ($datatype === 'album') {
		...
		if ($cmd === 'edit') {
			if (($album !== L10n::t('Profile Photos')) && ($album !== 'Contact Photos') && ($album !== L10n::t('Contact Photos'))) {
				if ($can_post) {
					$edit_tpl = Text::getMarkupTemplate('album_edit.tpl');

					$album_e = $album;

					$o .= App::replaceMacros($edit_tpl,[
		...
		$tpl = Text::getMarkupTemplate('photo_album.tpl');
		$o .= App::replaceMacros($tpl, [
			'$photos' => $photos,
			'$album' => $album,
			'$can_post' => $can_post,
	...
	if ($datatype === 'image') {
		...
		if ($cmd === 'edit') {
			$tpl = Text::getMarkupTemplate('photo_edit_head.tpl');
			$a->page['htmlhead'] .= App::replaceMacros($tpl,[
				'$prevlink' => $prevlink,
				'$nextlink' => $nextlink
			]);
		...
		if ($cmd === 'edit' && $can_post) {
			$edit_tpl = Text::getMarkupTemplate('photo_edit.tpl');

			$album_e = $ph[0]['album'];
			$caption_e = $ph[0]['desc'];
			$aclselect_e = ACL::getFullSelectorHTML($a->user, false, $ph[0]);

			$edit = App::replaceMacros($edit_tpl, [
				'$id' => $ph[0]['id'],
		...
		if (count($linked_items)) {
			$cmnt_tpl = Text::getMarkupTemplate('comment_item.tpl');
			$tpl = Text::getMarkupTemplate('photo_item.tpl');
			$return_path = $a->cmd;

			if ($can_post || Security::canWriteToUserWall($owner_uid)) {
				$like_tpl = Text::getMarkupTemplate('like_noshare.tpl');
				$likebuttons = App::replaceMacros($like_tpl, [
					'$id' => $link_item['id'],
			...
			if (!DBA::isResult($items)) {
				if (($can_post || Security::canWriteToUserWall($owner_uid))) {
					$comments .= App::replaceMacros($cmnt_tpl, [
						'$return_path' => '',
			...
			if (DBA::isResult($items)) {
				...
				foreach ($items as $item) {
					...
					$comments .= App::replaceMacros($template,[
						'$id' => $item['id'],
		...
		$photo_tpl = Text::getMarkupTemplate('photo_view.tpl');
		$o .= App::replaceMacros($photo_tpl, [
			'$id' => $ph[0]['id'],
	...
	$tpl = Text::getMarkupTemplate('photos_recent.tpl');
	$o .= App::replaceMacros($tpl, [

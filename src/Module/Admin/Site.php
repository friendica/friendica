...
class Site extends BaseAdminModule
{
	public static function post(array $parameters = [])
	{
		...
		if (!empty($_POST['relocate']) && !empty($_POST['relocate_url']) && $_POST['relocate_url'] != "") {
			...
			if (!is_array($parsed) || empty($parsed['host']) || empty($parsed['scheme'])) {
				...
				AppR::baseUrl()->redirect('admin/site');
			}
			...
			AppR::baseUrl()->saveByURL($new_url);
		...
		AppR::baseUrl()->redirect('admin/site' . $active_panel);
	}
	...
	public static function content(array $parameters = [])
	{
		...
		$diaspora_able = (AppR::baseUrl()->getUrlPath() == '');
		...
		return Renderer::replaceMacros($t, [
			...
			'$relocate_url'           => ['relocate_url', L10n::t('New base url'), AppR::baseUrl()->get(), L10n::t('Change base url for this server. Sends relocate message to all Friendica and Diaspora* contacts of all users.')],

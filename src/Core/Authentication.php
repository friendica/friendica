...
class Authentication extends BaseObject
{
	...
	public static function setAuthenticatedSessionForUser($user_record, $login_initial = false, $interactive = false, $login_refresh = false)
	{
		...
		if ($login_initial) {
			Text::logger('auth_identities: ' . print_r($a->identities, true), LOGGER_DEBUG);
		}
		if ($login_refresh) {
			Text::logger('auth_identities refresh: ' . print_r($a->identities, true), LOGGER_DEBUG);
		}
		...
		if ($login_initial) {
			...
			if (!empty($_SESSION['remember'])) {
				Text::logger('Injecting cookie for remembered user ' . $a->user['nickname']);

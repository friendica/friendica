...
class DBA
{
	...
	public static function p($sql) {
		...
		if ((substr_count($sql, '?') != count($args)) && (count($args) > 0)) {
			...
			App::logger('Parameter mismatch. Query "'.$sql.'" - Parameters '.print_r($args, true), LOGGER_DEBUG);
		}
		...
		if ((self::$errorno != 0) && !$called_from_e) {
			...
			App::logger('DB Error '.self::$errorno.': '.self::$error."\n".
			...
			if ($errorno == 2006) {
				if (self::$in_retrial || !self::reconnect()) {
					...
					if (self::$in_retrial) {
						App::logger('Giving up retrial because of database error '.$errorno.': '.$error);
					} else {
						App::logger("Couldn't reconnect after database error ".$errorno.': '.$error);
					}
					exit(1);
				} else {
					// We try it again
					App::logger('Reconnected after database error '.$errorno.': '.$error);
	...
	public static function e($sql) {
		...
		if (self::$errorno != 0) {
			...
			if ($errorno == 2006) {
				App::logger('Giving up because of database error '.$errorno.': '.$error);
				exit(1);
			}
	...
	public static function insert($table, $param, $on_duplicate_update = false) {
		...
		if (empty($table) || empty($param)) {
			App::logger('Table and fields have to be set');
			return false;
		}
	...
	public static function delete($table, array $conditions, array $options = [], $in_process = false, array &$callstack = [])
	{
		if (empty($table) || empty($conditions)) {
			App::logger('Table and conditions have to be set');
			return false;
		}
		...
		if (!$in_process) {
			...
			foreach ($commands AS $command) {
				...
				if ((count($command['conditions']) > 1) || is_int($first_key)) {
					...
					App::logger(self::replaceParameters($sql, $conditions), LOGGER_DATA);
			...
			foreach ($compacted AS $table => $values) {
				foreach ($values AS $field => $field_value_list) {
					foreach ($field_value_list AS $field_values) {
						...
						App::logger(self::replaceParameters($sql, $field_values), LOGGER_DATA);
	...
	public static function update($table, $fields, $condition, $old_fields = []) {
		...
		if (empty($table) || empty($fields) || empty($condition)) {
			App::logger('Table, fields and condition have to be set');
			return false;
		}

...
class PostUpdate
{
	...
	private static function update1194()
	{
		...
		App::logger("End ID: ".$end_id, LOGGER_DEBUG);
		...
		if (!$r) {
			...
			App::logger("Update is done", LOGGER_DEBUG);
			return true;
		} else {
		...
		App::logger("Start ID: ".$start_id, LOGGER_DEBUG);
		...
		App::logger("Progress: Start: ".$start_id." position: ".$pos_id." end: ".$end_id, LOGGER_DEBUG);

		q("UPDATE `item` ".$query2." SET `item`.`global` = 1 ".$query3,
			intval($start_id), intval($pos_id),
			DBA::escape(Protocol::DFRN), DBA::escape(Protocol::DIASPORA), DBA::escape(Protocol::OSTATUS));

		App::logger("Done", LOGGER_DEBUG);
	}
	...
	private static function update1279()
	{
		...
		App::logger("Start from item " . $id, LOGGER_DEBUG);
		...
		if (DBA::errorNo() != 0) {
			App::logger('Database error ' . DBA::errorNo() . ':' . DBA::errorMessage());
			return false;
		}
		...
		App::logger("Processed rows: " . $rows . " - last processed item:  " . $id, LOGGER_DEBUG);
		...
		if ($start_id == $id) {
			...
			foreach ($nullfields as $field) {
				$fields = [$field => null];
				$condition = [$field => ''];
				App::logger("Setting '" . $field . "' to null if empty.", LOGGER_DEBUG);
	...
	private static function update1281()
	{
		...
		if ($start_id == $id) {
			App::logger("Updating item-uri in item-activity", LOGGER_DEBUG);
			...
			App::logger("Updating item-uri in item-content", LOGGER_DEBUG);

...
class Database
{
	...
	public function __construct(IConfigCache $configCache, Profiler $profiler, LoggerInterface $logger, $serverAddr, $user, $pass, $db, $charset = null)
	{
		...
		if (!(strlen($server) && strlen($user))) {
			throw new \Exception(sprintf('No user and server for DB connection found.'));
		}
	}
	...
	public function connect()
	{
		...
		if (class_exists('\PDO') && in_array('mysql', PDO::getAvailableDrivers())) {
			$this->driver = 'pdo';
			$connect = "mysql:host=" . $this->db_server . ";dbname=" . $this->db_name;

			if ($this->db_port > 0) {
				$connect .= ";port=" . $this->db_port;
			}

			if ($this->db_charset) {
				$connect .= ";charset=" . $this->db_charset;
			}

			try {
				$this->connection = @new PDO($connect, $this->db_user, $this->db_pass);
		...
		if (!$this->connected && class_exists('\mysqli')) {
			...
			if ($this->db_port > 0) {
				$this->connection = @new mysqli($this->db_server, $this->db_user, $this->db_pass, $this->db_name, $this->db_port);
			} else {
				$this->connection = @new mysqli($this->db_server, $this->db_user, $this->db_pass, $this->db_pass);
			}
	...
	public function insert($table, $param, $on_duplicate_update = false)
	{
		...
		$sql = "INSERT INTO `" . $this->escape($table) . "` (`" . implode("`, `", array_keys($param)) . "`) VALUES (" .
	...
	public function lock($table)
	{
		...
		$success = $this->e("LOCK TABLES `" . $this->escape($table) . "` WRITE");

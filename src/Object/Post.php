...
class Post
{
	...
	public function getTemplateData($conv_responses, $thread_level = 1)
	{
		...
		$tmp_item = [
			...
			'name'            => $name_e,
			'thumb'           => $this->app->removeBaseURL(ProxyUtils::proxifyUrl($item['author-avatar'], false, ProxyUtils::SIZE_THUMB)),
			'osparkle'        => $osparkle,
			'sparkle'         => $sparkle,
			'title'           => $title_e,
			...
			'owner_photo'     => $this->app->removeBaseURL(ProxyUtils::proxifyUrl($item['owner-avatar'], false, ProxyUtils::SIZE_THUMB)),

	...
	private static function convertShareCallback(array $attributes, array $author_contact, $content, $is_quote_share, $simplehtml)
	{
		...
		switch ($simplehtml) {
			...
			case 3: // Diaspora
				...
				if (stripos(Text::normaliseLink($attributes['link']), 'http://twitter.com/') === 0) {
			...
			default:
				...
				if (stripos(Text::normaliseLink($attributes['link']), 'http://twitter.com/') === 0 && OEmbed::isAllowedURL($attributes['link'])) {
					try {
					...
					$tpl = Text::getMarkupTemplate('shared_content.tpl');
					$text .= App::replaceMacros($tpl, [

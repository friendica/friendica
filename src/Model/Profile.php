...
class Profile
{
	...
	public static function load(App $a, $nickname, $profile = 0, array $profiledata = [], $show_connect = true)
	{
		...
		if (!DBA::isResult($user) && empty($profiledata)) {
			App::logger('profile error: ' . $a->query_string, LOGGER_DEBUG);
	...
	private static function sidebar($profile, $block = 0, $show_connect = true)
	{
		...
		if (($profile['network'] != '') && ($profile['network'] != Protocol::DFRN)) {
			$profile['network_name'] = Text::formatNetworkName($profile['network'], $profile['url']);
		} else {
	...
	public static function zrlInit(App $a)
	{
		...
		if (!$cid) {
			App::logger('No contact record found for ' . $my_url, LOGGER_DEBUG);
			return;
		}
		...
		if (DBA::isResult($contact) && remote_user() && remote_user() == $contact['id']) {
			App::logger('The visitor ' . $my_url . ' is already authenticated', LOGGER_DEBUG);
			return;
		}
		...
		if (Cache::get($cachekey)) {
			App::logger('URL ' . $my_url . ' already tried to authenticate.', LOGGER_DEBUG);
			return;
		} else {
			Cache::set($cachekey, true, Cache::MINUTE);
		}

		App::logger('Not authenticated. Invoking reverse magic-auth for ' . $my_url, LOGGER_DEBUG);
		...
		if ($basepath != $a->getBaseURL() && !strstr($dest, '/magic') && !strstr($dest, '/rmagic')) {
			...
			if ($serverret->isSuccess()) {
				App::logger('Doing magic auth for visitor ' . $my_url . ' to ' . $magic_path, LOGGER_DEBUG);
	...
	public static function openWebAuthInit($token)
	{
		...
		if(!$cid) {
			App::logger('owt: unable to finger ' . $visitor_handle, LOGGER_DEBUG);
			return;
		}
		...
		App::logger('OpenWebAuth: auth success from ' . $visitor['addr'], LOGGER_DEBUG);
	}

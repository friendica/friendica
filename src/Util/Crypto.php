...
class Crypto
{
	...
	private static function pubRsaToMe($key, &$m, &$e)
	{
		...
		$m = Text::base64UrlDecode($r[0]->asnData[0]->asnData);
		$e = Text::base64UrlDecode($r[0]->asnData[1]->asnData);
	}
	...
	public static function pemToMe($key, &$m, &$e)
	{
		...
		$m = Text::base64UrlDecode($r[0]->asnData[1]->asnData[0]->asnData[0]->asnData);
		$e = Text::base64UrlDecode($r[0]->asnData[1]->asnData[0]->asnData[1]->asnData);
	}
	...
	private static function encapsulateOther($data, $pubkey, $alg)
	{
		...
		if (method_exists(__CLASS__, $fn)) {
			...
			$iv  = random_bytes(256);
			$result['data'] = Text::base64UrlEncode(self::$fn($data, $key, $iv), true);
			...
			$result['alg'] = $alg;
			$result['key'] = Text::base64UrlEncode($k, true);
			openssl_public_encrypt($iv, $i, $pubkey);
			$result['iv'] = Text::base64UrlEncode($i, true);
	...
	private static function encapsulateAes($data, $pubkey)
	{
		...
		$iv  = random_bytes(16);
		$result = ['encrypted' => true];
		$result['data'] = Text::base64UrlEncode(self::encryptAES256CBC($data, $key, $iv), true);
	...
	private static function unencapsulateOther($data, $prvkey, $alg)
	{
		...
		if (method_exists(__CLASS__, $fn)) {
			openssl_private_decrypt(Text::base64UrlDecode($data['key']), $k, $prvkey);
			openssl_private_decrypt(Text::base64UrlDecode($data['iv']), $i, $prvkey);

			return self::$fn(Text::base64UrlDecode($data['data']), $k, $i);
		} else {
	...
	private static function unencapsulateAes($data, $prvkey)
	{
		...
		return self::decryptAES256CBC(Text::base64UrlDecode($data['data']), $k, $i);
	}
}

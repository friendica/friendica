...
class DiasporaContact extends BaseRepository
{
	...
	public function save(Entity\DiasporaContact $DiasporaContact): Entity\DiasporaContact
	{
		...
		$fields = [
			...
			'baseurl'           => (string)$DiasporaContact->baseurl,
	...
	/**
	 * Fetches a Diaspora profile from a given URI and updates it depending on the mode
	 *
	 * @param WebFingerUri $uri    profile URI
	...
	public function getByAddr(WebFingerUri $uri, ?bool $update = self::UPDATE_IF_MISSING_OR_OUTDATED): Entity\DiasporaContact
	{
		...
		// Trigger a contact update if needed
		$contact = Contact::getByURL($uri, $update, ['uri-id', 'network', 'next-update']);
	...
	{
		try {
			$DiasporaContact = $this->selectOne(['guid' => $guid]);
			return (string)$DiasporaContact->url;
		} catch (HTTPException\NotFoundException $e) {
			return null;
		}
	}
	...
	{
		$contact = Contact::getByURL($uri, $update);

		if (empty($contact['uri-id'])) {
			throw new HTTPException\NotFoundException('Diaspora profile with URI ' . $uri . ' not found');
		}

		return $this->selectOneByUriId($contact['uri-id']);
	}
}

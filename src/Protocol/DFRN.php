...
class DFRN
{
	...
	public static function feed($dfrn_id, $owner_nick, $last_update, $direction = 0, $onlyheader = false)
	{
		...
		if (! DBA::isResult($r)) {
			App::logger(sprintf('No contact found for nickname=%d', $owner_nick), LOGGER_WARNING);
			killme();
		}
		...
		if (! $public_feed) {
			...
			if (! DBA::isResult($r)) {
				App::logger(sprintf('No contact found for uid=%d', $owner_id), LOGGER_WARNING);
				killme();
			}
		...
		if (isset($category)) {
			$sql_post_table = sprintf(
				...
				DBA::escape(Text::protectSprintf($category)),
				intval(TERM_OBJ_POST),
				intval(TERM_CATEGORY),
				intval($owner_id)
			);
			//$sql_extra .= Text::fileTagFileQuery('item',$category,'category');
		}
	...
	public static function deliver($owner, $contact, $atom, $dissolve = false, $legacy_transport = false)
	{
		...
		if (!$dissolve && !$legacy_transport) {
			...
			if ($curlResult >= 200) {
				App::logger('Delivery via Diaspora transport layer was successful with status ' . $curlResult);
				return $curlResult;
			}
		}
		...
		App::logger("Local rino version: ". $rino, LOGGER_DEBUG);
		...
		App::logger('dfrn_deliver: ' . $xml, LOGGER_DATA);
		...
		if (strpos($xml, '<?xml') === false) {
			App::logger('dfrn_deliver: no valid XML returned');
			App::logger('dfrn_deliver: returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		App::logger("Remote rino version: ".$rino_remote_version." for ".$contact["url"], LOGGER_DEBUG);
		...
		if ($final_dfrn_id != $orig_id) {
			App::logger('dfrn_deliver: wrong dfrn_id.');
		...
		if ($rino > 0 && $rino_remote_version > 0 && (! $dissolve)) {
			App::logger('rino version: '. $rino_remote_version);
			...
			switch ($rino_remote_version) {
				...
				default:
					App::logger("rino: invalid requested version '$rino_remote_version'");
					Contact::markForArchival($contact);
					return -8;
			}
			...
			App::logger('md5 rawkey ' . md5($postvars['key']));

			$postvars['key'] = bin2hex($postvars['key']);
		}


		App::logger('dfrn_deliver: ' . "SENDING: " . print_r($postvars, true), LOGGER_DATA);

		$postResult = Network::post($contact['notify'], $postvars);

		$xml = $postResult->getBody();

		App::logger('dfrn_deliver: ' . "RECEIVED: " . $xml, LOGGER_DATA);
		...
		if (strpos($xml, '<?xml') === false) {
			App::logger('dfrn_deliver: phase 2: no valid XML returned');
			App::logger('dfrn_deliver: phase 2: returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		if (!empty($res->message)) {
			App::logger('Delivery returned status '.$res->status.' - '.$res->message, LOGGER_DEBUG);
		}
	...
	public static function transmit($owner, $contact, $atom, $public_batch = false)
	{
		...
		if (!$public_batch) {
			if (empty($contact['addr'])) {
				App::logger('Empty contact handle for ' . $contact['id'] . ' - ' . $contact['url'] . ' - trying to update it.');
				...
				if (empty($contact['addr'])) {
					App::logger('Unable to find contact handle for ' . $contact['id'] . ' - ' . $contact['url']);
					Contact::markForArchival($contact);
					return -21;
				}
			}
			...
			if (empty($fcontact)) {
				App::logger('Unable to find contact details for ' . $contact['id'] . ' - ' . $contact['addr']);
				Contact::markForArchival($contact);
				return -22;
			}
		...
		$curl_stat = $postResult->getReturnCode();
		if (empty($curl_stat) || empty($xml)) {
			App::logger('Empty answer from ' . $contact['id'] . ' - ' . $dest_url);
			Contact::markForArchival($contact);
			return -9; // timed out
		}
		...
		if (strpos($xml, '<?xml') === false) {
			App::logger('No valid XML returned from ' . $contact['id'] . ' - ' . $dest_url);
			App::logger('Returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		if (!empty($res->message)) {
			App::logger('Transmit to ' . $dest_url . ' returned status '.$res->status.' - '.$res->message, LOGGER_DEBUG);
		}
	...
	private static function birthdayEvent($contact, $birthday)
	{
		...
		App::logger('updating birthday: ' . $birthday . ' for contact ' . $contact['id']);
	...
	private static function fetchauthor($xpath, $context, $importer, $element, $onlyfetch, $xml = "")
	{
		...
		$condition = ["`uid` = ? AND `nurl` = ? AND `network` != ?",
			$importer["importer_uid"], Text::normaliseLink($author["link"]), Protocol::STATUSNET];
		...
		if (DBA::isResult($contact_old)) {
			...
			if (!$onlyfetch) {
				App::logger("Contact ".$author["link"]." wasn't found for user ".$importer["importer_uid"]." XML: ".$xml, LOGGER_DEBUG);
			}
		...
		if (empty($author['avatar'])) {
			App::logger('Empty author: ' . $xml);
		}

		if (DBA::isResult($contact_old) && !$onlyfetch) {
			App::logger("Check if contact details for contact " . $contact_old["id"] . " (" . $contact_old["nick"] . ") have to be updated.", LOGGER_DEBUG);
			...
			App::logger("Hidden status for contact " . $contact_old["url"] . ": " . $hide, LOGGER_DEBUG);
			...
			foreach ($datefields as $field) {
				...
				if (strtotime($contact[$field]) > strtotime($contact_old[$field])) {
					App::logger("Difference for contact " . $contact["id"] . " in field '" . $field . "'. New value: '" . $contact[$field] . "', old value '" . $contact_old[$field] . "'", LOGGER_DEBUG);
					$update = true;
				}
			}
			...
			if ($update) {
				App::logger("Update contact data for contact " . $contact["id"] . " (" . $contact["nick"] . ")", LOGGER_DEBUG);
	...
	private static function processMail($xpath, $mail, $importer)
	{
		...
		App::logger("Mail is processed, notification was sent.");
	}
	...
	private static function processSuggestion($xpath, $suggestion, $importer)
	{
		...
		App::logger("Processing suggestions");
		 ...
		 */
		$condition = ['name' => $suggest["name"], 'nurl' => Text::normaliseLink($suggest["url"]),
			'uid' => $suggest["uid"]];
	...
	{
		App::logger("Processing relocations");
		...
		if (!DBA::isResult($r)) {
			App::logger("Query failed to execute, no result returned in " . __FUNCTION__);
			return false;
		}
		...
		$fields = ['name' => $relocate["name"], 'photo' => $relocate["avatar"],
			'url' => $relocate["url"], 'nurl' => Text::normaliseLink($relocate["url"]),
			'addr' => $relocate["addr"], 'connect' => $relocate["addr"],
			'notify' => $relocate["notify"], 'server_url' => $relocate["server_url"]];
		DBA::update('gcontact', $fields, ['nurl' => Text::normaliseLink($old["url"])]);
		...
		$condition = ["(`id` = ?) OR (`nurl` = ?)", $importer["id"], Text::normaliseLink($old["url"])];
	...
	private static function getEntryType($importer, $item)
	{
		if ($item["parent-uri"] != $item["uri"]) {
			...
			if ($importer["page-flags"] == Contact::PAGE_COMMUNITY || $importer["page-flags"] == Contact::PAGE_PRVGROUP) {
				$sql_extra = "";
				$community = true;
				App::logger("possible community action");
			} else {
			...
			if ($is_a_remote_action && $community && (!$r[0]["forum_mode"]) && (!$r[0]["wall"])) {
				$is_a_remote_action = false;
				App::logger("not a community action");
			}
	...
	private static function doPoke($item, $importer, $posted_id)
	{
		...
		if (($xo->type == ACTIVITY_OBJ_PERSON) && ($xo->id)) {
			...
			if ($Blink && Text::linkCompare($Blink, System::baseUrl() . "/profile/" . $importer["nickname"])) {
	...
	{
		App::logger("Process verb ".$item["verb"]." and object-type ".$item["object-type"]." for entrytype ".$entrytype, LOGGER_DEBUG);
	...
	private static function processEntry($header, $xpath, $entry, $importer, $xml)
	{
		...
		if (DBA::isResult($current) && !self::isEditedTimestampNewer($current, $item)) {
			App::logger("Item ".$item["uri"]." (".$item['edited'].") already existed.", LOGGER_DEBUG);
			return;
		}
		...
		$dsprsig = Text::unxmlify(XML::getFirstNodeValue($xpath, "dfrn:diaspora_signature/text()", $entry));
		if ($dsprsig != "") {
		...
		if ($entrytype == DFRN::REPLY_RC) {
			...
			if (($item["object-type"] == ACTIVITY_OBJ_EVENT) && !$owner_unknown) {
				App::logger("Item ".$item["uri"]." seems to contain an event.", LOGGER_DEBUG);
				$ev = Event::fromBBCode($item["body"]);
				if ((x($ev, "desc") || x($ev, "summary")) && x($ev, "start")) {
					App::logger("Event in item ".$item["uri"]." was found.", LOGGER_DEBUG);
					$ev["cid"]     = $importer["id"];
		...
		if (!self::processVerbs($entrytype, $importer, $item, $is_like)) {
			App::logger("Exiting because 'processVerbs' told us so", LOGGER_DEBUG);
			return;
		}
		...
		if (($entrytype == DFRN::TOP_LEVEL) && $owner_unknown) {
			App::logger("Item won't be stored because user " . $importer["importer_uid"] . " doesn't follow " . $item["owner-link"] . ".", LOGGER_DEBUG);
			return;
		}
		...
		if (DBA::isResult($current)) {
			if (self::updateContent($current, $item, $importer, $entrytype)) {
				App::logger("Item ".$item["uri"]." was updated.", LOGGER_DEBUG);
			} else {
				App::logger("Item " . $item["uri"] . " already existed.", LOGGER_DEBUG);
			}
			return;
		}
		...
		if (in_array($entrytype, [DFRN::REPLY, DFRN::REPLY_RC])) {
			...
			if ($posted_id) {
				App::logger("Reply from contact ".$item["contact-id"]." was stored with id ".$posted_id, LOGGER_DEBUG);
			...
			if (($importer["uid"] == 0) && ($importer["importer_uid"] != 0)) {
				App::logger("Contact ".$importer["id"]." isn't known to user ".$importer["importer_uid"].". The post will be ignored.", LOGGER_DEBUG);
				return;
			}
			if (!Text::linkCompare($item["owner-link"], $importer["url"])) {
				/*
				 ...
				 */
				App::logger('Correcting item owner.', LOGGER_DEBUG);
			...
			if (($importer["rel"] == Contact::FOLLOWER) && (!self::tgroupCheck($importer["importer_uid"], $item))) {
				App::logger("Contact ".$importer["id"]." is only follower and tgroup check was negative.", LOGGER_DEBUG);
				return;
			}
			...
			App::logger("Item was stored with id ".$posted_id, LOGGER_DEBUG);
	...
	{
		App::logger("Processing deletions");
		$uri = null;
		...
		if (!DBA::isResult($item)) {
			App::logger("Item with uri " . $uri . " for user " . $importer["importer_uid"] . " wasn't found.", LOGGER_DEBUG);
			return;
		}

		if (strstr($item['file'], '[')) {
			App::logger("Item with uri " . $uri . " for user " . $importer["importer_uid"] . " is filed. So it won't be deleted.", LOGGER_DEBUG);
			return;
		}
		...
		if (($item['id'] == $item['parent']) && ($item['contact-id'] != $importer["id"])) {
			App::logger("Item with uri " . $uri . " don't belong to contact " . $importer["id"] . " - ignoring deletion.", LOGGER_DEBUG);
			return;
		}
		...
		if (($item['id'] != $item['parent']) && ($item['contact-id'] != $importer["id"])) {
			...
			if (!Item::exists($condition)) {
				App::logger("Item with uri " . $uri . " wasn't found or mustn't be deleted by contact " . $importer["id"] . " - ignoring deletion.", LOGGER_DEBUG);
				return;
			}
		}
		...
		App::logger('deleting item '.$item['id'].' uri='.$uri, LOGGER_DEBUG);
	...
	public static function import($xml, $importer, $sort_by_date = false)
	{
		...
		App::logger("Import DFRN message for user " . $importer["importer_uid"] . " from contact " . $importer["id"], LOGGER_DEBUG);
		...
		App::logger("Import done for user " . $importer["importer_uid"] . " from contact " . $importer["id"], LOGGER_DEBUG);
		return 200;
	}
	...
	public static function autoRedir(App $a, $contact_nick)
	{
		...
		if (local_user()) {
			...
			$nurl = Text::normaliseLink($baseurl);
			...
			App::logger('auto_redir: ' . $r[0]['name'] . ' ' . $sec, LOGGER_DEBUG);
	...
	private static function tgroupCheck($uid, $item)
	{
		...
		$link = Text::normaliseLink(System::baseUrl() . '/profile/' . $user['nickname']);

		/*
		 * Diaspora uses their own hardwired link URL in @-tags
		 * instead of the one we supply with webfinger
		 */
		$dlink = Text::normaliseLink(System::baseUrl() . '/u/' . $user['nickname']);

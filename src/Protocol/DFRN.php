...
class DFRN
{
	...
	public static function feed($dfrn_id, $owner_nick, $last_update, $direction = 0, $onlyheader = false)
	{
		...
		if (! DBA::isResult($r)) {
			Text::logger(sprintf('No contact found for nickname=%d', $owner_nick), LOGGER_WARNING);
			killme();
		}
		...
		if (! $public_feed) {
			...
			if (! DBA::isResult($r)) {
				Text::logger(sprintf('No contact found for uid=%d', $owner_id), LOGGER_WARNING);
				killme();
			}
	...
	public static function deliver($owner, $contact, $atom, $dissolve = false, $legacy_transport = false)
	{
		...
		if (!$dissolve && !$legacy_transport) {
			...
			if ($curlResult >= 200) {
				Text::logger('Delivery via Diaspora transport layer was successful with status ' . $curlResult);
				return $curlResult;
			}
		}
		...
		Text::logger("Local rino version: ". $rino, LOGGER_DEBUG);
		...
		Text::logger('dfrn_deliver: ' . $xml, LOGGER_DATA);
		...
		if (strpos($xml, '<?xml') === false) {
			Text::logger('dfrn_deliver: no valid XML returned');
			Text::logger('dfrn_deliver: returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		Text::logger("Remote rino version: ".$rino_remote_version." for ".$contact["url"], LOGGER_DEBUG);
		...
		if ($final_dfrn_id != $orig_id) {
			Text::logger('dfrn_deliver: wrong dfrn_id.');
		...
		if ($rino > 0 && $rino_remote_version > 0 && (! $dissolve)) {
			Text::logger('rino version: '. $rino_remote_version);
			...
			switch ($rino_remote_version) {
				...
				default:
					Text::logger("rino: invalid requested version '$rino_remote_version'");
					Contact::markForArchival($contact);
					return -8;
			}
			...
			Text::logger('md5 rawkey ' . md5($postvars['key']));

			$postvars['key'] = bin2hex($postvars['key']);
		}


		Text::logger('dfrn_deliver: ' . "SENDING: " . print_r($postvars, true), LOGGER_DATA);

		$postResult = Network::post($contact['notify'], $postvars);

		$xml = $postResult->getBody();

		Text::logger('dfrn_deliver: ' . "RECEIVED: " . $xml, LOGGER_DATA);
		...
		if (strpos($xml, '<?xml') === false) {
			Text::logger('dfrn_deliver: phase 2: no valid XML returned');
			Text::logger('dfrn_deliver: phase 2: returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		if (!empty($res->message)) {
			Text::logger('Delivery returned status '.$res->status.' - '.$res->message, LOGGER_DEBUG);
		}
	...
	public static function transmit($owner, $contact, $atom, $public_batch = false)
	{
		...
		if (!$public_batch) {
			if (empty($contact['addr'])) {
				Text::logger('Empty contact handle for ' . $contact['id'] . ' - ' . $contact['url'] . ' - trying to update it.');
				...
				if (empty($contact['addr'])) {
					Text::logger('Unable to find contact handle for ' . $contact['id'] . ' - ' . $contact['url']);
					Contact::markForArchival($contact);
					return -21;
				}
			}
			...
			if (empty($fcontact)) {
				Text::logger('Unable to find contact details for ' . $contact['id'] . ' - ' . $contact['addr']);
				Contact::markForArchival($contact);
				return -22;
			}
		...
		$curl_stat = $postResult->getReturnCode();
		if (empty($curl_stat) || empty($xml)) {
			Text::logger('Empty answer from ' . $contact['id'] . ' - ' . $dest_url);
			Contact::markForArchival($contact);
			return -9; // timed out
		}
		...
		if (strpos($xml, '<?xml') === false) {
			Text::logger('No valid XML returned from ' . $contact['id'] . ' - ' . $dest_url);
			Text::logger('Returned XML: ' . $xml, LOGGER_DATA);
			Contact::markForArchival($contact);
			return 3;
		}
		...
		if (!empty($res->message)) {
			Text::logger('Transmit to ' . $dest_url . ' returned status '.$res->status.' - '.$res->message, LOGGER_DEBUG);
		}
	...
	private static function birthdayEvent($contact, $birthday)
	{
		...
		Text::logger('updating birthday: ' . $birthday . ' for contact ' . $contact['id']);
	...
	private static function fetchauthor($xpath, $context, $importer, $element, $onlyfetch, $xml = "")
	{
		...
		if (DBA::isResult($contact_old)) {
			...
			if (!$onlyfetch) {
				Text::logger("Contact ".$author["link"]." wasn't found for user ".$importer["importer_uid"]." XML: ".$xml, LOGGER_DEBUG);
			}
		...
		if (empty($author['avatar'])) {
			Text::logger('Empty author: ' . $xml);
		}

		if (DBA::isResult($contact_old) && !$onlyfetch) {
			Text::logger("Check if contact details for contact " . $contact_old["id"] . " (" . $contact_old["nick"] . ") have to be updated.", LOGGER_DEBUG);
			...
			Text::logger("Hidden status for contact " . $contact_old["url"] . ": " . $hide, LOGGER_DEBUG);
			...
			foreach ($datefields as $field) {
				...
				if (strtotime($contact[$field]) > strtotime($contact_old[$field])) {
					Text::logger("Difference for contact " . $contact["id"] . " in field '" . $field . "'. New value: '" . $contact[$field] . "', old value '" . $contact_old[$field] . "'", LOGGER_DEBUG);
					$update = true;
				}
			}
			...
			if ($update) {
				Text::logger("Update contact data for contact " . $contact["id"] . " (" . $contact["nick"] . ")", LOGGER_DEBUG);
	...
	private static function processMail($xpath, $mail, $importer)
	{
		...
		Text::logger("Mail is processed, notification was sent.");
	}
	...
	private static function processSuggestion($xpath, $suggestion, $importer)
	{
		...
		Text::logger("Processing suggestions");
	...
	{
		Text::logger("Processing relocations");
		...
		if (!DBA::isResult($r)) {
			Text::logger("Query failed to execute, no result returned in " . __FUNCTION__);
			return false;
		}
	...
	private static function getEntryType($importer, $item)
	{
		if ($item["parent-uri"] != $item["uri"]) {
			...
			if ($importer["page-flags"] == Contact::PAGE_COMMUNITY || $importer["page-flags"] == Contact::PAGE_PRVGROUP) {
				$sql_extra = "";
				$community = true;
				Text::logger("possible community action");
			} else {
			...
			if ($is_a_remote_action && $community && (!$r[0]["forum_mode"]) && (!$r[0]["wall"])) {
				$is_a_remote_action = false;
				Text::logger("not a community action");
			}
	...
	{
		Text::logger("Process verb ".$item["verb"]." and object-type ".$item["object-type"]." for entrytype ".$entrytype, LOGGER_DEBUG);
	...
	{
		Text::logger("Processing entries");
		...
		if (DBA::isResult($current) && !self::isEditedTimestampNewer($current, $item)) {
			Text::logger("Item ".$item["uri"]." (".$item['edited'].") already existed.", LOGGER_DEBUG);
			return;
		}
		...
		if ($entrytype == DFRN::REPLY_RC) {
			...
			if (($item["object-type"] == ACTIVITY_OBJ_EVENT) && !$owner_unknown) {
				Text::logger("Item ".$item["uri"]." seems to contain an event.", LOGGER_DEBUG);
				$ev = Event::fromBBCode($item["body"]);
				if ((x($ev, "desc") || x($ev, "summary")) && x($ev, "start")) {
					Text::logger("Event in item ".$item["uri"]." was found.", LOGGER_DEBUG);
					$ev["cid"]     = $importer["id"];
		...
		if (!self::processVerbs($entrytype, $importer, $item, $is_like)) {
			Text::logger("Exiting because 'processVerbs' told us so", LOGGER_DEBUG);
			return;
		}
		...
		if (($entrytype == DFRN::TOP_LEVEL) && $owner_unknown) {
			Text::logger("Item won't be stored because user " . $importer["importer_uid"] . " doesn't follow " . $item["owner-link"] . ".", LOGGER_DEBUG);
			return;
		}
		...
		if (DBA::isResult($current)) {
			if (self::updateContent($current, $item, $importer, $entrytype)) {
				Text::logger("Item ".$item["uri"]." was updated.", LOGGER_DEBUG);
			} else {
				Text::logger("Item " . $item["uri"] . " already existed.", LOGGER_DEBUG);
			}
			return;
		}
		...
		if (in_array($entrytype, [DFRN::REPLY, DFRN::REPLY_RC])) {
			...
			if ($posted_id) {
				Text::logger("Reply from contact ".$item["contact-id"]." was stored with id ".$posted_id, LOGGER_DEBUG);
			...
			if (($importer["uid"] == 0) && ($importer["importer_uid"] != 0)) {
				Text::logger("Contact ".$importer["id"]." isn't known to user ".$importer["importer_uid"].". The post will be ignored.", LOGGER_DEBUG);
				return;
			}
			if (!Text::linkCompare($item["owner-link"], $importer["url"])) {
				 ...
				 */
				Text::logger('Correcting item owner.', LOGGER_DEBUG);
			...
			if (($importer["rel"] == Contact::FOLLOWER) && (!self::tgroupCheck($importer["importer_uid"], $item))) {
				Text::logger("Contact ".$importer["id"]." is only follower and tgroup check was negative.", LOGGER_DEBUG);
				return;
			}
			...
			Text::logger("Item was stored with id ".$posted_id, LOGGER_DEBUG);
	...
	{
		Text::logger("Processing deletions");
		$uri = null;
		...
		if (!DBA::isResult($item)) {
			Text::logger("Item with uri " . $uri . " for user " . $importer["importer_uid"] . " wasn't found.", LOGGER_DEBUG);
			return;
		}

		if (strstr($item['file'], '[')) {
			Text::logger("Item with uri " . $uri . " for user " . $importer["importer_uid"] . " is filed. So it won't be deleted.", LOGGER_DEBUG);
			return;
		}
		...
		if (($item['id'] == $item['parent']) && ($item['contact-id'] != $importer["id"])) {
			Text::logger("Item with uri " . $uri . " don't belong to contact " . $importer["id"] . " - ignoring deletion.", LOGGER_DEBUG);
			return;
		}
		...
		if (($item['id'] != $item['parent']) && ($item['contact-id'] != $importer["id"])) {
			...
			if (!Item::exists($condition)) {
				Text::logger("Item with uri " . $uri . " wasn't found or mustn't be deleted by contact " . $importer["id"] . " - ignoring deletion.", LOGGER_DEBUG);
				return;
			}
		}
		...
		Text::logger('deleting item '.$item['id'].' uri='.$uri, LOGGER_DEBUG);
	...
	public static function import($xml, $importer, $sort_by_date = false)
	{
		...
		Text::logger("Import DFRN message for user " . $importer["importer_uid"] . " from contact " . $importer["id"], LOGGER_DEBUG);
		...
		Text::logger("Import done for user " . $importer["importer_uid"] . " from contact " . $importer["id"], LOGGER_DEBUG);
		return 200;
	}
	...
	public static function autoRedir(App $a, $contact_nick)
	{
		...
		if (local_user()) {
			...
			Text::logger('auto_redir: ' . $r[0]['name'] . ' ' . $sec, LOGGER_DEBUG);

...
class DFRN
{
	...
	public static function feed($dfrn_id, $owner_nick, $last_update, $direction = 0, $onlyheader = false)
	{
		...
		if (!DBM::isResult($items) || $onlyheader) {
	...
	private static function entry($doc, $type, $item, $owner, $comment = false, $cid = 0, $single = false)
	{
		...
		foreach ($mentioned as $mention) {
			...
			if (DBM::isResult($r) && ($r[0]["forum"] || $r[0]["prv"])) {
				XML::addElement(
					$doc,
					$entry,
					"link",
	...
	private static function processEntry($header, $xpath, $entry, $importer, $xml)
	{
		...
		if (DBM::isResult($current) && edited_timestamp_is_newer($current[0], $item)
	...
	public static function autoRedir(App $a, $contact_nick)
	{
		...
		if (local_user()) {
			...
			if ((! DBM::isResult($r)) || $r[0]['id'] == remote_user()) {
				return;
			}

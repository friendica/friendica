...
class DFRN
{
	...
	private static function addHeader($doc, $owner, $authorelement, $alternatelink = "", $public = false)
	{
		...
		if ($owner['page-flags'] == Profile::PAGE_COMMUNITY) {
		...
		/// @todo We need a way to transmit the different page flags like "Profile::PAGE_PRVGROUP"
	...
	public static function deliver($owner, $contact, $atom, $dissolve = false)
	{
		...
		$page         = (($owner['page-flags'] == Profile::PAGE_COMMUNITY) ? 1 : 0);

		logger("Remote rino version: ".$rino_remote_version." for ".$contact["url"], LOGGER_DEBUG);

		if ($owner['page-flags'] == Profile::PAGE_PRVGROUP) {
			$page = 2;
		}
		...
		if (($contact['duplex'] && strlen($contact['pubkey']))
			|| ($owner['page-flags'] == Profile::PAGE_COMMUNITY && strlen($contact['pubkey']))
		...
		if ((($contact['rel']) && ($contact['rel'] != Contact::SHARING) && (! $contact['blocked'])) || ($owner['page-flags'] == Profile::PAGE_COMMUNITY)) {
			$postvars['data'] = $atom;
			$postvars['perm'] = 'rw';
		} else {
		...
		if ($rino > 0 && $rino_remote_version > 0 && (! $dissolve)) {
			...
			if ($dfrn_version >= 2.1) {
				...
				if (($contact['duplex'] && strlen($contact['prvkey'])) || ($owner['page-flags'] == Profile::PAGE_COMMUNITY)) {
					openssl_private_encrypt($key, $postvars['key'], $contact['prvkey']);
				} else {
					openssl_public_encrypt($key, $postvars['key'], $contact['pubkey']);
				}
	...
	private static function getEntryType($importer, $item)
	{
		if ($item["parent-uri"] != $item["uri"]) {
			...
			if ($importer["page-flags"] == Profile::PAGE_COMMUNITY || $importer["page-flags"] == Profile::PAGE_PRVGROUP) {
				$sql_extra = "";
				$community = true;
	...
	private static function tgroupCheck($uid, $item)
	{
		...
		$community_page = ($u[0]['page-flags'] == Profile::PAGE_COMMUNITY);
		$prvgroup = ($u[0]['page-flags'] == Profile::PAGE_PRVGROUP);

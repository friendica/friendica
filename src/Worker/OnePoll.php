...
class OnePoll
{
	public static function execute($contact_id = 0, $command = '')
	{
		...
		if (!DBA::isResult($contact)) {
			App::logger('Contact not found or cannot be used.');
			return;
		}
		...
		if ($contact['archive'] || ($contact["network"] == Protocol::DIASPORA) || ($contact["rel"] == Contact::FOLLOWER)) {
			...
			if ($last_updated) {
				App::logger('Contact '.$contact['id'].' had last update on '.$last_updated, LOGGER_DEBUG);
				...
				App::logger('Contact '.$contact['id'].' is marked for archival', LOGGER_DEBUG);
			}
		...
		if (($contact['network'] === Protocol::OSTATUS) || ($contact['network'] === Protocol::DIASPORA) || ($contact['network'] === Protocol::DFRN)) {
			if (!PortableContact::reachable($contact['url'])) {
				App::logger("Skipping probably dead contact ".$contact['url']);
			...
			if (!Contact::updateFromProbe($contact["id"])) {
				Contact::markForArchival($contact);
				App::logger('Contact is marked dead');
		...
		if ($importer_uid == 0) {
			App::logger('Ignore public contacts');
		...
		if (!DBA::isResult($r)) {
			App::logger('No self contact for user '.$importer_uid);
		...
		App::logger("poll: ({$contact['network']}-{$contact['id']}) IMPORTER: {$importer['name']}, CONTACT: {$contact['name']}");
		...
		if ($contact['network'] === Protocol::DFRN) {
			...
			App::logger('handshake with url ' . $url . ' returns xml: ' . $handshake_xml, LOGGER_DATA);

			if (!strlen($handshake_xml) || ($html_code >= 400) || !$html_code) {
				App::logger("$url appears to be dead - marking for death ");
			...
			if (!strstr($handshake_xml, '<')) {
				App::logger('response from ' . $url . ' did not contain XML.');
			...
			if (intval($res->status) == 1) {
				App::logger("$url replied status 1 - marking for death ");
				...
				App::logger("$url back from the dead - removing mark for death");
				Contact::unmarkForArchival($contact);
			}
			...
			if ($final_dfrn_id != $orig_id) {
				...
				App::logger('ID did not decode: ' . $contact['id'] . ' orig: ' . $orig_id . ' final: ' . $final_dfrn_id);
			...
			App::logger("Mail: Fetching for ".$contact['addr'], LOGGER_DEBUG);
			...
			App::logger("Mail: Enabled", LOGGER_DEBUG);
			...
			if (DBA::isResult($user) && DBA::isResult($mailconf)) {
				...
				unset($password);
				App::logger("Mail: Connect to " . $mailconf['user']);
				if ($mbox) {
					$fields = ['last_check' => DateTimeFormat::utcNow()];
					DBA::update('mailacct', $fields, ['id' => $mailconf['id']]);
					App::logger("Mail: Connected to " . $mailconf['user']);
				} else {
					App::logger("Mail: Connection error ".$mailconf['user']." ".print_r(imap_errors(), true));
				}
			}
			...
			if ($mbox) {
				...
				if (count($msgs)) {
					App::logger("Mail: Parsing ".count($msgs)." mails from ".$contact['addr']." for ".$mailconf['user'], LOGGER_DEBUG);

					$metas = Email::messageMeta($mbox, implode(',', $msgs));

					if (count($metas) != count($msgs)) {
						App::logger("for " . $mailconf['user'] . " there are ". count($msgs) . " messages but received " . count($metas) . " metas", LOGGER_DEBUG);
					} else {
						$msgs = array_combine($msgs, $metas);

						foreach ($msgs as $msg_uid => $meta) {
							App::logger("Mail: Parsing mail ".$msg_uid, LOGGER_DATA);
							...
							if (DBA::isResult($item)) {
								App::logger("Mail: Seen before ".$msg_uid." for ".$mailconf['user']." UID: ".$importer_uid." URI: ".$datarray['uri'],LOGGER_DEBUG);
								...
								switch ($mailconf['action']) {
									case 0:
										App::logger("Mail: Seen before ".$msg_uid." for ".$mailconf['user'].". Doing nothing.", LOGGER_DEBUG);
										break;
									case 1:
										App::logger("Mail: Deleting ".$msg_uid." for ".$mailconf['user']);
										imap_delete($mbox, $msg_uid, FT_UID);
										break;
									case 2:
										App::logger("Mail: Mark as seen ".$msg_uid." for ".$mailconf['user']);
										imap_setflag_full($mbox, $msg_uid, "\\Seen", ST_UID);
										break;
									case 3:
										App::logger("Mail: Moving ".$msg_uid." to ".$mailconf['movetofolder']." for ".$mailconf['user']);
										imap_setflag_full($mbox, $msg_uid, "\\Seen", ST_UID);
							...
							if (!$r) {
								App::logger("Mail: can't fetch msg ".$msg_uid." for ".$mailconf['user']);
								continue;
							}
							$datarray['body'] = Text::escapeTags($r['body']);
							$datarray['body'] = BBCode::limitBodySize($datarray['body']);

							App::logger("Mail: Importing ".$msg_uid." for ".$mailconf['user']);
					...
					App::logger("Mail: no mails for ".$mailconf['user']);
				}

				App::logger("Mail: closing connection for ".$mailconf['user']);
				imap_close($mbox);
			}
		}

		if ($xml) {
			App::logger('received xml : ' . $xml, LOGGER_DATA);
			if (!strstr($xml, '<')) {
				App::logger('post_handshake: response from ' . $url . ' did not contain XML.');
			...
			App::logger("Consume feed of contact ".$contact['id']);
			...
			App::logger("Contact ".$contact['id']." returned hub: ".$hub." Network: ".$contact['network']." Relation: ".$contact['rel']." Update: ".$hub_update);

			if (strlen($hub) && $hub_update && (($contact['rel'] != Contact::FOLLOWER) || $contact['network'] == Protocol::FEED)) {
				App::logger('hub ' . $hubmode . ' : ' . $hub . ' contact name : ' . $contact['name'] . ' local user : ' . $importer['name']);
				$hubs = explode(',', $hub);

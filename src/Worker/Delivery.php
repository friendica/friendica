...
class Delivery
{
	...
	public static function execute($cmd, $target_id, $contact_id)
	{
		...
		if ($cmd == self::MAIL) {
			...
			$localhost = App::baseUrl()->getHostname();
			if (strpos($localhost, ':')) {
	...
	private static function deliverDFRN($cmd, $contact, $owner, $items, $target_item, $public_message, $top_level, $followup)
	{
		...
		if (Strings::compareLink($basepath, App::baseUrl())) {
	...
	private static function deliverMail($cmd, $contact, $owner, $target_item, $thr_parent)
	{
		...
		if (($contact['rel'] == Model\Contact::FRIEND) && !$contact['blocked']) {
			...
			$headers  = 'From: '. Email::encodeHeader($local_user['username'], 'UTF-8') . ' <noreply@' . App::baseUrl()->getHostname() . '>' . "\n";
		}

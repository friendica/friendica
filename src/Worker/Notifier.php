...
class Notifier
{
	public static function execute($cmd, $item_id)
	{
		...
		App::logger('notifier: invoked: '.$cmd.': '.$item_id, LOGGER_DEBUG);
		...
		if ($cmd == Delivery::MAIL) {
			...
			foreach ($inboxes as $inbox) {
				App::logger('Account removal for user ' . $item_id . ' to ' . $inbox .' via ActivityPub', LOGGER_DEBUG);
			...
			if ((count($items) == 1) && ($items[0]['id'] === $target_item['id']) && ($items[0]['uri'] === $items[0]['parent-uri'])) {
				App::logger('notifier: top level post');
				$top_level = true;
			}
		}
		...
		if (!in_array($cmd, [Delivery::MAIL, Delivery::SUGGESTION, Delivery::RELOCATION])) {
			...
			App::logger('GUID: '.$target_item["guid"].': Parent is '.$parent['network'].'. Thread parent is '.$thr_parent['network'], LOGGER_DEBUG);
			...
			if ($relay_to_owner) {
				...
				App::logger('notifier: followup '.$target_item["guid"].' to '.$conversant_str, LOGGER_DEBUG);
				...
				App::logger("Notify ".$target_item["guid"]." via PuSH: ".($push_notify?"Yes":"No"), LOGGER_DEBUG);
			} else {
				$followup = false;

				App::logger('Distributing directly '.$target_item["guid"], LOGGER_DEBUG);

				// don't send deletions onward for other people's stuff

				if ($target_item['deleted'] && !intval($target_item['wall'])) {
					App::logger('notifier: ignoring delete notification for non-wall item');
					return;
				}
				...
				if (count($url_recipients)) {
					App::logger('notifier: '.$target_item["guid"].' url_recipients ' . print_r($url_recipients,true));
				}
			...
			if (($thr_parent && ($thr_parent['network'] == Protocol::OSTATUS)) || ($parent['network'] == Protocol::OSTATUS)) {
				...
				App::logger('Some parent is OStatus for '.$target_item["guid"]." - Author: ".$thr_parent['author-id']." - Owner: ".$thr_parent['owner-id'], LOGGER_DEBUG);

				// Send a salmon to the parent author
				$probed_contact = DBA::selectFirst('contact', ['url', 'notify'], ['id' => $thr_parent['author-id']]);
				if (DBA::isResult($probed_contact) && !empty($probed_contact["notify"])) {
					App::logger('Notify parent author '.$probed_contact["url"].': '.$probed_contact["notify"]);
				...
				if (DBA::isResult($probed_contact) && !empty($probed_contact["notify"])) {
					App::logger('Notify parent owner '.$probed_contact["url"].': '.$probed_contact["notify"]);
					$url_recipients[$probed_contact["notify"]] = $probed_contact["notify"];
				}
				...
				foreach ($arr as $x) {
					//App::logger('Checking tag '.$x, LOGGER_DEBUG);
					$matches = null;
					if (preg_match('/@\[url=([^\]]*)\]/',$x,$matches)) {
							$probed_contact = Probe::uri($matches[1]);
						if ($probed_contact["notify"] != "") {
							App::logger('Notify mentioned user '.$probed_contact["url"].': '.$probed_contact["notify"]);
							$url_recipients[$probed_contact["notify"]] = $probed_contact["notify"];
						}
					}
				}
		...
		if (DBA::isResult($r)) {
			foreach ($r as $contact) {
				App::logger("Deliver ".$item_id." to ".$contact['url']." via network ".$contact['network'], LOGGER_DEBUG);
		...
		if (!Config::get('system', 'dfrn_only') && count($url_recipients) && ($public_message || $push_notify) && $normal_mode) {
			...
			foreach ($url_recipients as $url) {
				if ($url) {
					App::logger('notifier: urldelivery: ' . $url);
		...
		if ($public_message) {
			...
			if (DBA::isResult($r)) {
				App::logger('pubdeliver '.$target_item["guid"].': '.print_r($r,true), LOGGER_DEBUG);
				...
				foreach ($r as $rr) {
					...
					if (($rr['network'] !== Protocol::DIASPORA) && (in_array($rr['id'], $conversants))) {
						App::logger('notifier: already delivered id=' . $rr['id']);
						continue;
					}

					if (!in_array($cmd, [Delivery::MAIL, Delivery::SUGGESTION]) && !$followup) {
						App::logger('notifier: delivery agent: '.$rr['name'].' '.$rr['id'].' '.$rr['network'].' '.$target_item["guid"]);
						Worker::add(['priority' => $a->queue['priority'], 'created' => $a->queue['created'], 'dont_fork' => true],
		...
		if ($push_notify) {
			App::logger('Activating internal PuSH for item '.$item_id, LOGGER_DEBUG);

			// Handling the pubsubhubbub requests
			PushSubscriber::publishFeed($owner['uid'], $a->queue['priority']);
		}

		App::logger('notifier: calling hooks for ' . $cmd . ' ' . $item_id, LOGGER_DEBUG);
	...
	private static function activityPubDelivery($a, $cmd, $item_id, $uid, $target_item, $parent)
	{
		...
		if ($target_item['origin']) {
			...
			App::logger('Origin item ' . $item_id . ' with URL ' . $target_item['uri'] . ' will be distributed.', LOGGER_DEBUG);
		} elseif (!DBA::exists('conversation', ['item-uri' => $target_item['uri'], 'protocol' => Conversation::PARCEL_ACTIVITYPUB])) {
			App::logger('Remote item ' . $item_id . ' with URL ' . $target_item['uri'] . ' is no AP post. It will not be distributed.', LOGGER_DEBUG);
			return;
		} else {
			// Remote items are transmitted via the personal inboxes.
			// Doing so ensures that the dedicated receiver will get the message.
			$personal = true;
			App::logger('Remote item ' . $item_id . ' with URL ' . $target_item['uri'] . ' will be distributed.', LOGGER_DEBUG);
		}
		...
		if (empty($inboxes)) {
			App::logger('No inboxes found for item ' . $item_id . ' with URL ' . $target_item['uri'] . '. It will not be distributed.', LOGGER_DEBUG);
			return;
		}
		...
		foreach ($inboxes as $inbox) {
			App::logger('Deliver ' . $item_id .' to ' . $inbox .' via ActivityPub', LOGGER_DEBUG);

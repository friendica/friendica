...
function api_source()
{
	...
	if (!empty($_SERVER['HTTP_USER_AGENT'])) {
		...
		App::logger("Unrecognized user-agent ".$_SERVER['HTTP_USER_AGENT'], LOGGER_DEBUG);
	} else {
		App::logger("Empty user-agent", LOGGER_DEBUG);
	}
	...
	try {
		foreach ($API as $p => $info) {
			if (strpos($a->query_string, $p) === 0) {
				...
				App::logger('API call for ' . $a->user['username'] . ': ' . $a->query_string);
				App::logger('API parameters: ' . print_r($_REQUEST, true));

				$stamp =  microtime(true);
				$return = call_user_func($info['func'], $type);
				$duration = (float) (microtime(true) - $stamp);
				App::logger("API call duration: " . round($duration, 2) . "\t" . $a->query_string, LOGGER_DEBUG);
		...
		App::logger('API call not implemented: ' . $a->query_string);
...
function api_get_user(App $a, $contact_id = null)
{
	...
	App::logger("api_get_user: Fetching user data for user ".$contact_id, LOGGER_DEBUG);
	...
	App::logger("api_get_user: user ".$user, LOGGER_DEBUG);
	...
	App::logger('api_user: ' . $extra_query . ', user: ' . $user);
...
function api_statuses_mediap($type)
{
	...
	if (api_user() === false) {
		App::logger('api_statuses_update: no user');
		throw new ForbiddenException();
	}
	$user_info = api_get_user($a);
	...
	if (!$parent) {
		...
		if ($throttle_day > 0) {
			...
			if ($posts_day > $throttle_day) {
				App::logger('Daily posting limit reached for user '.api_user(), LOGGER_DEBUG);
		...
		if ($throttle_week > 0) {
			...
			if ($posts_week > $throttle_week) {
				App::logger('Weekly posting limit reached for user '.api_user(), LOGGER_DEBUG);
		...
		if ($throttle_month > 0) {
			...
			if ($posts_month > $throttle_month) {
				App::logger('Monthly posting limit reached for user '.api_user(), LOGGER_DEBUG);
...
function api_media_upload()
{
	...
	App::logger("Media uploaded: " . print_r($returndata, true), LOGGER_DEBUG);
...
function api_status_show($type, $item_id = 0)
{
	...
	App::logger('api_status_show: user_info: '.print_r($user_info, true), LOGGER_DEBUG);
	...
	if (DBA::isResult($lastwall)) {
		...
		App::logger('status_info: '.print_r($status_info, true), LOGGER_DEBUG);
...
function api_statuses_show($type)
{
	...
	App::logger('API: api_statuses_show: ' . $id);
...
function api_conversation_show($type)
{
	...
	App::logger('API: api_conversation_show: '.$id);
...
function api_statuses_repeat($type)
{
	...
	App::logger('API: api_statuses_repeat: '.$id);
...
function api_statuses_destroy($type)
{
	...
	App::logger('API: api_statuses_destroy: '.$id);
...
function api_favorites($type)
{
	...
	App::logger('api_favorites: self:' . $user_info['self']);
...
function api_friendships_destroy($type)
{
	...
	if (empty($contact_id)) {
		App::logger("No user_id specified", LOGGER_DEBUG);
	...
	if(!DBA::isResult($contact)) {
		App::logger("No contact found for ID" . $contact_id, LOGGER_DEBUG);
	...
	if (!DBA::isResult($contact)) {
		App::logger("Not following Contact", LOGGER_DEBUG);
		throw new NotFoundException("Not following Contact");
	}

	if (!in_array($contact['network'], Protocol::NATIVE_SUPPORT)) {
		App::logger("Not supported", LOGGER_DEBUG);
	...
	if ($owner) {
		...
		App::logger("No owner found", LOGGER_DEBUG);
...
function save_media_to_database($mediatype, $media, $type, $album, $allow_cid, $deny_cid, $allow_gid, $deny_gid, $desc, $profile = 0, $visibility = false, $photo_id = null)
{
	...
	if ($maximagesize && ($filesize > $maximagesize)) {
		$formattedBytes = Text::formatBytes($maximagesize);
	...
	if ($max_length > 0) {
		...
		App::logger("File upload: Scaling picture to new size " . $max_length, LOGGER_DEBUG);
	}
	...
	if ($mediatype == "photo") {
		...
		if ($width > 320 || $height > 320) {
			...
			if (!$r) {
				App::logger("photo upload: image upload with scale 2 (320x320) failed");
			}
		}
		App::logger("photo upload: new photo upload ended", LOGGER_DEBUG);
	} elseif ($mediatype == "profileimage") {
		// upload profile image (scales 4, 5, 6)
		App::logger("photo upload: starting new profile image upload", LOGGER_DEBUG);

		if ($width > 300 || $height > 300) {
			$Image->scaleDown(300);
			$r = Photo::store($Image, local_user(), $visitor, $hash, $filename, $album, 4, $profile, $allow_cid, $allow_gid, $deny_cid, $deny_gid, $desc);
			if (!$r) {
				App::logger("photo upload: profile image upload with scale 4 (300x300) failed");
			}
		}
		...
		if ($width > 80 || $height > 80) {
			...
			if (!$r) {
				App::logger("photo upload: profile image upload with scale 5 (80x80) failed");
			}
		}
		...
		if ($width > 48 || $height > 48) {
			...
			if (!$r) {
				App::logger("photo upload: profile image upload with scale 6 (48x48) failed");
			}
		}
		$Image->__destruct();
		App::logger("photo upload: new profile image upload ended", LOGGER_DEBUG);
	}
...
function api_friendica_remoteauth()
{
	...
	App::logger($contact['name'] . ' ' . $sec, LOGGER_DEBUG);
...
function api_in_reply_to($item)
{
	...
	if (($item['thr-parent'] != $item['uri']) && (intval($item['parent']) != intval($item['id']))) {
		...
		if (intval($in_reply_to['status_id']) == intval($item['id'])) {
			App::logger('this message should never appear: id: '.$item['id'].' similar to reply-to: '.$in_reply_to['status_id'], LOGGER_DEBUG);
			$in_reply_to['status_id'] = null;
			$in_reply_to['user_id'] = null;
			$in_reply_to['status_id_str'] = null;
			$in_reply_to['user_id_str'] = null;

...
function localize_item(&$item)
{
	...
	*/
	...
	if (Text::activityMatch($item['verb'], ACTIVITY_FRIEND)) {
		...
		$links = XML::parseString($xmlhead."<links>".Text::unxmlify($obj->link)."</links>");
...
function conversation(App $a, array $items, Pager $pager, $mode, $update, $preview = false, $order = 'commented', $uid = 0)
{
	...
	$page_template = Text::getMarkupTemplate("conversation.tpl");
	...
	if (!empty($items)) {
		...
		if (in_array($mode, ['network-new', 'search', 'contact-posts'])) {
			...
			foreach ($items as $item) {
				...
				$body = Text::prepareBody($item, true, $preview);
			...
			// Normal View
			$page_template = Text::getMarkupTemplate("threaded_conversation.tpl");
			...
			if (!$threads) {
				App::logger('[ERROR] conversation : Failed to get template data.', LOGGER_DEBUG);
				$threads = [];
			}
		}
	}

	$o = App::replaceMacros($page_template, [
...
function item_photo_menu($item) {
	...
	$network = '';
	$rel = 0;
	$condition = ['uid' => local_user(), 'nurl' => Text::normaliseLink($item['author-link'])];
...
function format_like($cnt, array $arr, $type, $id) {
	...
	$phrase .= EOL ;
	$o .= App::replaceMacros(Text::getMarkupTemplate('voting_fakelink.tpl'), [
		'$phrase' => $phrase,
		'$type' => $type,
		'$id' => $id
	]);
...
function status_editor(App $a, $x, $notes_cid = 0, $popup = false)
{
	...
	$geotag = x($x, 'allow_location') ? App::replaceMacros(Text::getMarkupTemplate('jot_geotag.tpl'), []) : '';
	...
	// $tpl = App::replaceMacros($tpl,array('$jotplugins' => $jotplugins));

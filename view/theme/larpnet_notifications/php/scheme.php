...
function larpnet_notifications_get_scheme_info($scheme)
{
...
function larpnet_notifications_scheme_get_list(): array
{
	$schemes = [
	...
	foreach (glob('view/theme/larpnet_notifications/scheme/*.php') ?: [] as $file) {
		$scheme = basename($file, '.php');
		if (!in_array($scheme, ['default', 'light', 'dark', 'black'])) {
			$scheme_info = larpnet_notifications_get_scheme_info($scheme);
...
function larpnet_notifications_scheme_get_current()
{
	$available = array_keys(larpnet_notifications_scheme_get_list());
...
function larpnet_notifications_scheme_get_current_for_user(int $uid)
{
	$available = array_keys(larpnet_notifications_scheme_get_list());

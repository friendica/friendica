 ...
 * Author: Spring <https://collegeintheclouds.org>
 * Maintainer: David Spring <collegeintheclouds.org>
 * Description: "Spring" is a simplified version of the Vier theme. It includes four color options and combines CSS3 with HTML5 and CSS Grid.
 */
...
function spring_init(App $a)
{
	...
	if (
		...
		spring_community_info();

		DI::page()['htmlhead'] .= "<link rel='stylesheet' type='text/css' href='view/theme/spring/wide.css' media='screen and (min-width: 1300px)'/>\n";
	}

	if (DI::mode()->isMobile() || DI::mode()->isMobile()) {
		DI::page()['htmlhead'] .= '<meta name=viewport content="width=device-width, initial-scale=1">'."\n";
		DI::page()['htmlhead'] .= '<link rel="stylesheet" type="text/css" href="view/theme/spring/mobile.css" media="screen"/>'."\n";
	}
	/// @todo deactivated since it doesn't work with desktop browsers at the moment
	//DI::page()['htmlhead'] .= '<link rel="stylesheet" type="text/css" href="view/theme/spring/mobile.css" media="screen and (max-width: 1000px)"/>'."\n";

	DI::page()['htmlhead'] .= <<< EOT
<link rel='stylesheet' type='text/css' href='view/theme/spring/narrow.css' media='screen and (max-width: 1100px)' />
...
EOT;
	...
	if (empty(DI::page()['aside']) && in_array($args->get(0), ["community", "calendar", "help", "delegation", "notifications",
		...
		DI::page()['htmlhead'] .= "<link rel='stylesheet' href='view/theme/spring/hide.css' />";
	}
}

function get_spring_config($key, $default = false, $admin = false)
{
	if (DI::userSession()->getLocalUserId() && !$admin) {
		$result = DI::pConfig()->get(DI::userSession()->getLocalUserId(), "spring", $key);
		if (!is_null($result)) {
			return $result;
		}
	}

	$result = DI::config()->get("spring", $key);
	if (!is_null($result)) {
		return $result;
	}
...
function spring_community_info()
{
	$show_pages      = get_spring_config("show_pages", 1);
	$show_profiles   = get_spring_config("show_profiles", 1);
	$show_helpers    = get_spring_config("show_helpers", 1);
	$show_services   = get_spring_config("show_services", 1);
	$show_friends    = get_spring_config("show_friends", 1);
	$show_lastusers  = get_spring_config("show_lastusers", 1);
 ...
 */
function spring_get_background_color(int $uid = null)
{
	return null;
}
 ...
 * @TODO Implement this function
 */
function spring_get_theme_color(int $uid = null)
{
	return null;
}

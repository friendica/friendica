	...
	<div class="contact-wrapper media" id="contact-entry-wrapper-{{$contact.id}}">
		...
		<div class="contact-photo-wrapper dropdown media-left">
			<div class="contact-entry-photo mframe" id="contact-entry-photo-{{$contact.id}}">
				...
				<div class="contact-photo-image-wrapper hidden-xs">
					<img class="contact-photo media-object xl" src="{{$contact.thumb}}" alt="{{$contact.name}}" />
				</div>
				...
				<button type="button" class="btn btn-link dropdown-toggle visible-xs" id="contact-photo-menu-button{{$contact.id}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					{{* use a smaller picture on very small displays (e.g. mobiles) *}}
					<div class="contact-photo-image-wrapper visible-xs">
						<img class="contact-photo-xs media-object" src="{{$contact.thumb}}" alt="{{$contact.name}}" />
		...
		<div class="media-body">
			...
			<div class="contact-entry-desc">
				<div class="contact-entry-name" id="contact-entry-name-{{$contact.id}}">
					...
					{{if $account_type}} <small class="contact-entry-details" id="contact-entry-accounttype-{{$contact.id}}">({$account_type})</small>{{/if}}
					{{if $account_type == 'Forum'}}<i class="fa fa-comments-o" aria-hidden="true"></i>{{/if}}
					{{* @todo this needs some changing in core because $contact.account_type contains a translated string which may notbe the same in every language *}}
					</h4>
				</div>
				{{if $alt_text}}<div class="contact-entry-details" id="contact-entry-rel-{{$contact.id}}" >{{$alt_text}}</div>{{/if}}
				{{if $itemurl}}<div class="contact-entry-details" id="contact-entry-url-{{$contact.id}}" >{{$itemurl}}</div>{{/if}}
				{{if $tags}}<div class="contact-entry-details" id="contact-entry-tags-{{$contact.id}}" >{{$tags}}</div>{{/if}}
				{{if $details}}<div class="contact-entry-details" id="contact-entry-details-{{$contact.id}}" >{{$details}}</div>{{/if}}
				{{if $network}}<div class="contact-entry-details" id="contact-entry-network-{{$contact.id}}" >{{$network}}</div>{{/if}}
			</div>
